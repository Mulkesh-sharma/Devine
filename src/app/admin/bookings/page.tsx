...
import { FiCalendar, FiUser, FiClock, FiCheck, FiX, FiEye, FiFilter, FiTrash2 } from 'react-icons/fi';
...
export default function AdminBookingsPage() {
  ...
  useEffect(() => {
    ...
    fetchBookings();
  }, [isAuthenticated, user, filterStatus, authLoading]);

  const fetchBookings = async () => {
    try {
      setLoading(true);
      let url = '/api/bookings?sort=date-desc';
      if (filterStatus !== 'all') {
  ...
  const handleStatusUpdate = async (status: string) => {
    ...
    try {
      const res = await fetch(`/api/bookings/${actionBooking._id}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <PageLayout title="Manage Bookings" subtitle="View and manage all pooja bookings">
      <Section>
        <div className="flex gap-2 mb-6 overflow-x-auto p-4">
          {['all', 'pending', 'confirmed', 'completed', 'cancelled'].map((status) => (
            <Button
              ...
              size="sm"
              className={filterStatus === status ? '' : 'text-gray-400 hover:text-white hover:bg-gray-800'}
            >
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-700">
                <tr>
                  ...
                  <th className="p-4 font-semibold text-gray-400 dark:text-gray-300">Customer</th>
                    ...
                    <tr key={booking._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm text-gray-400">{booking.contactPerson.phone}</span>
                        </div>
                      </td>
        ...
        >
          <MenuItem onClick={() => handleStatusUpdate('confirmed')}>Mark Confirmed</MenuItem>
          <MenuItem onClick={() => handleStatusUpdate('completed')}>Mark Completed</MenuItem>
          <MenuItem onClick={() => handleStatusUpdate('cancelled')} className="text-orange-500">Cancel Booking</MenuItem>
          <Divider />
          <MenuItem onClick={handleDeleteBooking} className="text-red-600 font-medium flex items-center gap-2">
